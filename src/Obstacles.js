let obstacles = {
    ...
    update: function () {
        ...
        for (let i = 0, size = this._obs.length; i < size; i++) {
            ...
            obs.x -= speed;
            // Collision
            if ((Ufo.x + RIGHT_COLLISION_TOLERANCE) < (obs.x + this.widths[obs.number]) &&
                (Ufo.x + Ufo.width) >= (obs.x + LEFT_COLLISION_TOLERANCE) &&
                (Ufo.y + Ufo.height) >= (ground.y - this.heights[obs.number])) {
